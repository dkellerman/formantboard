...
export function ApiPage() {
  ...
  return (
    <section className={cn("mx-auto flex w-[95vw] max-w-5xl flex-col gap-6 pb-10 text-sm")}>
      <header className={cn("rounded-lg border border-zinc-300 bg-sky-50 p-5")}>
        <h2 className={cn("m-0 text-2xl font-semibold text-zinc-900")}>FormantBoard API</h2>
        <p className={cn("mb-0 mt-2 text-zinc-700")}>
          Use <code>window.fb</code> to schedule notes and shape vowels while playing.
        </p>
      </header>

      <section className={cn("rounded-lg border border-zinc-300 bg-white p-5")}>
        <h3 className={cn("m-0 text-lg font-semibold")}>AI Discovery</h3>
        <p className={cn("mb-0 mt-2 text-zinc-700")}>
          For autonomous agents, publish these stable entrypoints:
        </p>
        <ul className={cn("mb-0 mt-2 list-disc space-y-1 pl-5 text-zinc-700")}>
          <li>
      ...
      <section className={cn("rounded-lg border border-zinc-300 bg-white p-5")}>
        <h3 className={cn("m-0 text-lg font-semibold")}>Quick Start</h3>
        <ol className={cn("mb-0 mt-3 list-decimal space-y-1 pl-5 text-zinc-700")}>
      ...
      <section className={cn("rounded-lg border border-zinc-300 bg-white p-5")}>
        <h3 className={cn("m-0 text-lg font-semibold")}>Core Methods</h3>
        <div className={cn("mt-3 overflow-x-auto")}>
          <table className={cn("w-full border-collapse text-left")}>
            <thead>
              <tr className={cn("border-b border-zinc-200 text-zinc-600")}>
      ...
      <section className={cn("rounded-lg border border-zinc-300 bg-white p-5")}>
        <h3 className={cn("m-0 text-lg font-semibold")}>Method Parameters</h3>
        <p className={cn("mb-0 mt-2 text-zinc-700")}>
          Expand each method to see exact params and object shapes.
        </p>
        <p className={cn("mb-0 mt-1 font-mono text-xs text-zinc-700")}>
          SupportedIPA = {SUPPORTED_IPA_VOWELS_TEXT}
        </p>
        <details className={cn("mt-2 rounded border border-zinc-200 bg-zinc-50 p-3")}>
          <summary className={cn("cursor-pointer font-medium text-zinc-900")}>
            Zod Schema Snapshot
          </summary>
          <p className={cn("mb-0 mt-2 text-xs text-zinc-700")}>
            These are the same schemas exposed at <code>window.fb.schemaJson</code>.
          </p>
          <p className={cn("mb-0 mt-1 text-xs text-zinc-700")}>
            Note: <code>window.fb.schemas</code> are live Zod objects and are not meant to be
            JSON-stringified.
          </p>
          <p className={cn("mb-0 mt-2 font-mono text-xs text-zinc-900")}>playEvents</p>
          <pre
            className={cn(
              "mt-1 max-h-40 overflow-auto rounded border border-zinc-200 bg-white p-2 font-mono text-[11px] text-zinc-800",
            )}
          >
            {PLAY_EVENTS_SCHEMA_JSON}
          </pre>
          <p className={cn("mb-0 mt-2 font-mono text-xs text-zinc-900")}>jsonPayload</p>
          <pre
            className={cn(
              "mt-1 max-h-40 overflow-auto rounded border border-zinc-200 bg-white p-2 font-mono text-[11px] text-zinc-800",
            )}
        ...
        <div className="mt-3 space-y-2">
          {METHOD_PARAM_DOCS.map((doc) => (
            <details
              key={doc.method}
              className={cn("rounded border border-zinc-200 bg-zinc-50 p-3")}
            >
              <summary className={cn("cursor-pointer font-mono text-sm text-zinc-900")}>
                {doc.method}
              </summary>
              <p className={cn("mb-0 mt-2 text-zinc-700")}>{doc.summary}</p>
              {doc.caveat ? (
                <p className={cn("mb-0 mt-1 text-xs text-rose-700")}>{doc.caveat}</p>
              ) : null}
              <p className={cn("mb-0 mt-2 font-mono text-xs text-zinc-800")}>{doc.signature}</p>
              <div className={cn("mt-2 overflow-x-auto rounded border border-zinc-200 bg-white")}>
                  ...
                  <tbody>
                    {doc.params.map((param) => (
                      <tr
                        ...
                        className={cn("border-b border-zinc-100")}
                      >
                        <td className={cn("px-2 py-1.5 font-mono text-zinc-900")}>{param.name}</td>
                        <td className={cn("px-2 py-1.5 font-mono text-zinc-800")}>{param.type}</td>
                        <td className={cn("px-2 py-1.5 text-zinc-800")}>
                          {param.required ? "yes" : "no"}
                        </td>
                        <td className={cn("px-2 py-1.5 text-zinc-700")}>{param.description}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {doc.shape ? (
                <pre
                  className={cn(
                    "mt-2 overflow-x-auto rounded border border-zinc-200 bg-white p-2 font-mono text-xs text-zinc-800",
                  )}
                >
                  {doc.shape}
                </pre>
      ...
      <section className={cn("rounded-lg border border-zinc-300 bg-white p-5")}>
        <h3 className={cn("m-0 text-lg font-semibold")}>Instructions for AI</h3>
        <details className={cn("mt-3 rounded border border-zinc-200 bg-zinc-50 p-3")}>
          <summary className={cn("cursor-pointer font-medium text-zinc-900")}>
          ...
          <p className={cn("mb-0 mt-3 text-zinc-700")}>
          ...
          <p className={cn("mb-0 mt-3 font-mono text-xs text-zinc-800")}>
      ...
      <section className={cn("rounded-lg border border-zinc-300 bg-white p-5")}>
        <h3 className={cn("m-0 text-lg font-semibold")}>Live JSON Sandbox</h3>
        <p className={cn("mb-0 mt-2 text-zinc-700")}>
        ...
        <textarea
          className={cn(
            "mt-3 h-[360px] w-full rounded border border-zinc-300 p-3 font-mono text-xs",
          )}
          value={payload}
